...
def process_sequence(SEQ):
    ...
    GT    = f"{POSES}/{SEQ}.txt"
    OUTPUT_PLOT = f"traj_surf_stereo_{SEQ}.png"
    ...
    # SURF detector with higher Hessian threshold for more features
    # hessianThreshold: lower = more features (default ~400)
    # nOctaves: number of pyramid octaves (default 4)
    ...
    # Use BFMatcher with L2 norm (SURF uses 64 or 128-dimensional float descriptors)
    ...
    print(f"Running CPU SURF Stereo VO on KITTI {SEQ}")
    ...
    for i, fname in enumerate(imgs):
        ...
        kpL, dL = surf.detectAndCompute(imgL, None)
        kpR, dR = surf.detectAndCompute(imgR, None)
    ...
    plt.plot(traj_est_aligned[:,0], traj_est_aligned[:,2],
             lw=2, color="red", label="CPU SURF Stereo VO")
    plt.axis("equal")
    plt.xlabel("X [m]")
    plt.ylabel("Z [m]")
    plt.title(f"KITTI {SEQ} – Stereo SURF Visual Odometry")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
if __name__ == "__main__":
    print("\nCPU SURF STEREO VISUAL ODOMETRY – MULTI SEQUENCE\n")
